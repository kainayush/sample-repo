import { getMetersByFeeder, getActiveBreakEvent, createBreakEvent, resolveBreakEvents } from '../db/database';
...
export function runBreakDetection(feederId: string, stalenessMs = DEFAULT_STALENESS_WINDOW_MS): DetectionResult {
  const now = new Date();
  const timestamp = now.toISOString();
  const trace: TraceStep[] = [];

  // Step 1: Collect meters and latest readings
  const rawMeters = getMetersByFeeder(feederId) as any[];
  ...
  const sortedMeters: MeterReadingState[] = rawMeters
    ...
    .map((m) => {
      ...
      let is_dead = false;
      let dead_reason: string | null = null;
  ...
  const activeBreak = getActiveBreakEvent(feederId) as any;
  ...
  if (transitionIndex !== -1) {
    ...
    if (!activeBreak || activeBreak.last_healthy_meter_id !== (lastHealthyMeter?.id || 'SUBSTATION') || activeBreak.first_dead_meter_id !== firstDeadMeter.id) {
      if (activeBreak) {
        resolveBreakEvents(feederId);
      }
      const newEvent = createBreakEvent(
        feederId,
        lastHealthyMeter ? lastHealthyMeter.id : 'SUBSTATION',
        firstDeadMeter.id
      );
